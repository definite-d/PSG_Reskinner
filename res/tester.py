...
def all_elements_demo():
    ...
    layout_r = [[name('Canvas'), sg.Canvas(background_color=sg.theme_button_color()[1], size=(125, 40))],
                ...
                [name('Tab, TabGroup'), sg.TabGroup([[sg.Tab('Tab1', [[sg.T(s=(15, 2))]]), sg.Tab('Tab2', [[]])]], right_click_menu=[[''], ['asdfe', 'adsfasdfadfs', ';klhjlkjh']])],
    ...
    layout = [[Menu(menudef, k='-CUST MENUBAR-', p=0)],
              ...
              [sg.Checkbox('Use Custom Titlebar & Menubar', use_custom_titlebar, enable_events=True,
                           k='-USE CUSTOM TITLEBAR-', p=0, tooltip='checkcheckcheck')],
              [sg.Col(layout_l, p=0, scrollable=True), sg.Col(layout_r, p=0)]]

    window = sg.Window('The PySimpleGUI Element List', [[sg.Col(layout, scrollable=True, key='allfather')]], finalize=True,
                       right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_EXIT,
                       # keep_on_top=True,
                       use_custom_titlebar=use_custom_titlebar, resizable=True)
    ...
    while True:
        ...
        if event == '-COMBO-':
            rs.animated_reskin(window, values['-COMBO-'], sg.theme, sg.LOOK_AND_FEEL_TABLE, honor_previous=False)
